...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-jbpm-custom-workitem-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-jbpm-custom-workitem-service apps/sample-jbpm-custom-workitem-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-jbpm-custom-workitem-service:1.0-SNAPSHOT
    oc expose svc/sample-jbpm-custom-workitem-service
else

	echo "Launching the application locally..."
	pattern="sample-jbpm-custom-workitem-service"
	files=( $pattern )
	cd ${files[0]}
